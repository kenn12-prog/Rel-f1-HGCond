# Step4_no_convolution: Graph compression using KMeans clustering (skip step3 convolution)
# Uses step2 node embeddings directly instead of step3 convolved features
...
class GraphCompressor:
    ...
    @staticmethod
    def create_compressed_dataset(step2_data: Dict,
                                original_batches: List[Dict], 
                                task,
                                compressor: 'GraphCompressor') -> Tuple[HeteroData, Dict[str, torch.Tensor]]:
        """
        Create compressed graph dataset using step2 embeddings (skip step3 convolution)
        """
        print("Creating compressed dataset using step2 embeddings (no convolution)...")
        ...
        try:
            ...
            if target_node_type in step2_data['node_embeddings']:
                # Map labels to correct nodes using global_node_mapping
                global_mapping = step2_data['global_node_mapping'][target_node_type]
                num_nodes = step2_data['node_counts'][target_node_type]
        ...
        compressed_features, compressed_labels, cluster_assignments = compressor.compress_graph_with_labels(
            step2_data['node_embeddings'],  # Use step2 embeddings instead of step3 convolved features
            labels_dict,
            step2_data['global_node_mapping']
        )
...
def save_compressed_graph(compressed_data, compressed_labels, dataset_name, task_name, compression_rate):
    ...
    filename = f"{dataset_name}_{task_name}_compressed_no_conv_{compression_rate:.3f}.pth"
    ...
    save_data = {
        ...
        'method': 'no_convolution'  # Mark this as no convolution version
    }
...
def load_compressed_graph(dataset_name, task_name, compression_rate):
    ...
    print(f"  Method: {save_data.get('method', 'unknown')}")
...
def main():
    """
    Main function for step4_no_convolution: Create and save compressed graph using step2 embeddings
    """
    print("=== Step4_no_convolution: Graph Compression (Skip Step3 Convolution) ===")
    ...
    # Load step2 results instead of step3
    print("\nLoading step2 results...")
    step2_data_path = Path(f"./saved_embeddings/{args.dataset}_{args.task}_embeddings_step2.pth")
    if not step2_data_path.exists():
        raise FileNotFoundError(f"Step2 results not found at {step2_data_path}. Please run step2 first.")
    
    step2_batches = torch.load(step2_data_path, map_location='cpu', weights_only=False)
    
    print(f"Loaded step2 results:")
    print(f"  Number of batches: {len(step2_batches)}")
    
    # Aggregate batch data to global features (same as step4 but using step2 data)
    print("\nAggregating batch data to global features...")
    
    # Get node types from first batch
    node_types = []
    if step2_batches:
        for node_type in step2_batches[0].keys():
            if node_type != 'edge_index_dict' and isinstance(step2_batches[0][node_type], dict):
                if 'embeddings' in step2_batches[0][node_type]:
    ...
    # Create step2_data format compatible with compression function
    step2_data = {
        'node_embeddings': global_features,  # Use different key to distinguish from step3
    ...
    print(f"\nCompressing graph with rate {args.compression_rate} (using step2 embeddings)...")
    ...
    compressed_data, compressed_labels = GraphCompressor.create_compressed_dataset(
        step2_data, step2_batches, task, compressor
    )
    
    print(f"\nGraph compression completed:")
    for node_type in compressed_data.node_types:
        if node_type in step2_data['node_counts']:
            original_count = step2_data['node_counts'][node_type]
    ...
    print(f"\nStep4_no_convolution completed successfully!")
    print(f"Compressed graph saved to: {save_path}")
    print(f"This baseline skips step3 convolution and uses step2 embeddings directly.")
