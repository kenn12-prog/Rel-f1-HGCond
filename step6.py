...
# Import CompressedGNN from step5
from step5 import CompressedGNN
...
class HybridModel(nn.Module):
    """
    Hybrid model: combines original dataset's encoder+temporal_encoder with compressed graph trained GNN
    
    Key insight: CompressedGNN was trained on encoder+temporal_encoder features, so can be used directly
    """
    def __init__(self, original_model: Model, compressed_gnn: CompressedGNN, 
        ...
        # Use complete compressed graph GNN (core achievement from step5 training)
        self.compressed_gnn = compressed_gnn
        ...
        for param in self.compressed_gnn.parameters():
            param.requires_grad = False
    ...
    def forward(self, batch, entity_table):
        ...
        # Step 5: Directly use compressed graph GNN for prediction
        # CompressedGNN was trained on exactly these features!
        with torch.no_grad():  # compressed graph GNN is also frozen
            # If convolution version, need to provide edge info; if MLP version, don't need
            # This structure was determined during step5 training
            if hasattr(self.compressed_gnn, 'use_convolution') and self.compressed_gnn.use_convolution:
                # Convolution version: create self-loop edges
                ...
                pred = self.compressed_gnn(target_features, edge_index)
            else:
                # MLP version: directly input features
                pred = self.compressed_gnn(target_features)
...
def load_models():
    ...
    compressed_gnn = CompressedGNN(
        input_dim=args.channels,  # 与原始模型的channels一致
        hidden_dim=step5_args.channels,
        output_dim=out_channels,
        use_convolution=step5_args.use_convolution,
    ...
    print(f"  Use convolution: {step5_args.use_convolution}")
